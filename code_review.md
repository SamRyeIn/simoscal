...
> **2026-08-18 — one deliberate exception to "findings are never deleted."**
> A private GUI client that this library serves moved to its own repository
> before `simoscal` opened to beta testers. Six findings raised against files
> that left with it — CR-20260724-05, CR-20260724-14, CR-20260813-03,
> CR-20260813-04, CR-20260815-01, CR-20260815-03 — keep their heading and index
> row here but had their bodies replaced with a pointer, and one review that
> raised no library findings at all was reduced to the two conclusions that do
> bear on this repository. Nothing else was removed. Every ID still resolves, so
> the `CR-` references in library code and in the index remain intact, and the
> unabridged text of all of it is preserved in the client repository's own copy
> of this file. Findings against library code were **not** touched, including
> those first noticed through the client — CR-20260813-01/02/05,
> CR-20260815-02/04/05 and CR-20260816-01 are all still here in full.
...
## Review 2026-07-24 — renderer-independent build service (`simoscal/tune/build_service.py`)
...
## Review 2026-07-24 — packaging, recovery, editing, and bridge continuation

- **Scope:** committed client-programme foundation through `284af5a`, plus Claude's
...
- **Method:** traced the V1/V4/V5/V6 acceptance criteria through packaging,
  recovery, generic editing, the Python JSON boundary, and the client facade;
...
- **Verification:** complete Python suite **655 passed** with the same four
  expected `StaleChecksumWarning`s. The client's Kotlin source
  and instrumentation source compiled under the documented JDK 17, building the
...
This finding was raised against client code that moved to its own
repository on 2026-08-18. Kept here as an index entry only, so the numbering
stays continuous and no cross-reference dangles; the full text, including the
evidence and the fix, is in the client repository's own copy of this file.
...
## Review 2026-07-24 — bridge independent cross-family pass (Opus)
...
The plan's cross-runtime golden gate — per-operation host-vs-embedded-runtime byte-
identical request/response fixtures — still does not exist, and its embedded leg
...
## Review 2026-08-13 — editing guards, staging immutability, and client integration
...
- **Method:** traced import → preflight → session → edit → recovery → build →
  ...
  paths, ran the client's JVM/manifest/package gates, ran the focused Python bridge
...
### CR-20260813-03 — Imports block the client's main thread — Medium, CONFIRMED — Fixed (2026-08-14)
...
text, the client forwarded it unchanged as `bin_name`, and
...
- **CR-20260813-03** — `ImportStore.importFile` is now `suspend` and wraps the
  ...
  remaining gap below.
- **CR-20260813-04** — The client's UI state gained `dirtyDraft`, `canMutateSession`,
...
## Review 2026-08-15 — first run on real hardware

- **Scope:** the first execution of the client on real arm64 hardware —
  a Samsung tablet (Android 16, `arm64-v8a`) — exercising
...
### CR-20260815-03 — Test-count table in the client README is stale — Low, CONFIRMED — Fixed (2026-08-15)
...
## Review 2026-08-16 — closing the cross-runtime parity gate *(moved 2026-08-18)*

This review's subject — an APK's shipped ABIs and the packaging around them —
left with the client repository on 2026-08-18. It raised **no findings** against
library code, so nothing in this log's index depends on it. The full text is in
the client repository's own copy of this file.

Two of its conclusions are worth keeping here because they are about *this*
library's parity payload, and a future agent would otherwise rediscover them:

- **A green `PARITY: MATCH` banner is not sufficient evidence; the step contents
  are.** The `boost_curve` leg records `{"skipped": …}` rather than failing when
  its switch-patch fixtures are absent, and because the digest is a sha256 over
  the whole `compared` dict, a host and a second runtime that both skip agree at
  a self-consistent digest and print a match. Assert on the steps.
- **A verdict does not survive a change to the artifact it was taken against.**
  Re-measure rather than inheriting a prior digest.

<details>
<summary>Original scope line, retained for provenance</summary>
...
- **Scope:** `199acc6` — the reversal of `4adc78c`. Removes
  ...
  (the client README, `implementation_details.md`, this file).
...
Severity is High because of what the failure costs. The client
's `recoverSession` clears the pointer on any failure — deliberately, so a bad
